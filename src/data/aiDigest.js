...
export const AI_DIGEST = [
  {
    ...
    risk:
      '1）目前是「內部目標／期望規模」，不是已遞交的 S-1 或已訂價區間，數字被市場當成既成事實的風險很高。2）SpaceX 紀錄本身會隨時間改寫，對標口徑（募資額、市值、還是整筆估值）報導未必統一。3）若後續財測、安全事件或監管（模型出口管制、自願安全測試）不及預期，超大 IPO 敘事可能迅速縮水。4）循環融資質疑（晶片商同時是客戶與股東）若在路演被放大，發行窗口可能推遲。非正式招股書，不宜當買賣依據。',
    tone: 'good',
  },
  {
    rank: 2,
    ...
    risk:
      '1）「逾 20%」可能只適用特定檔位（輸入／輸出、快取、批次、承諾用量），完整價目表尚未被所有二級來源核對。2）若為限時或僅限新客戶，對產業均價的衝擊會被高估。3）降價若伴隨更嚴的 rate limit 或較高的安全監控成本（先前曾有約 20% 推論監控 overhead 的討論），開發者實際帳單不一定下降。4）價格戰可能迫使對手跟降，短線利多用量、中線利空模型公司估值。請以官網價目與原文為準。',
    tone: 'good',
  },
  {
    rank: 3,
    ...
    risk:
      '1）關鍵字是「擬／計畫／據傳」，金額、支付時程（一次或多年授權）、是否含股權都可能在定稿前大幅修改。2）The Information 近日亦有輝達相關報導被公司否認（中國 LPU）的前例，二級來源互相引用時要以公司聲明覆核。3）歐盟與美國對「授權掩蓋併購」的審查可能拉長。4）挖角若觸發競業與 IP 訴訟，實際交付權重的時間會晚於頭條。在交割或 8-K／官方確認前，不宜把 60 億當成已入帳 capex。',
    tone: 'good',
  },
  {
    rank: 4,
    ...
    risk:
      '1）ARC-AGI-3 是受控的遊戲式推理基準，外推到企業 ERP、客服、寫程式會過度樂觀。2）100% 成績高度依賴特定 harness＋特定底模組合，換模型可能大幅回落。3）Nvidia 是開放 agent 棧的利害關係人，論文與公關時程重疊。4）多加一層監督代理會增加延遲與 token，實帳成本未必下降。5）資安上，監督者本身也可能被越獄或與主 agent 共謀（今年已有沙盒逃逸、Hugging Face 憑證竊取等案例）。不要把「harness＞模型」讀成可以買便宜小模型就結束。',
    tone: 'good',
  },
  {
    rank: 5,
    ...
    risk:
      '1）挖角是意圖，不是 tape-out 日曆；沒有公開的製程節點、年出貨量或代工廠。2）Google 出身不代表能把 TPU 產能或 IP 帶走，競業與商业秘密是現實障礙。3）Anthropic 若加深 Google Cloud／TPU 綁定，反而可能減少對輝達的短單，對「全面利多半導體」的讀法不成立。4）自研晶片失敗率高，歷史上多數模型公司的 ASIC 計畫會縮成「多雲議價籌碼」。短線不宜當台積電或智原的即時催化。',
    tone: 'neutral',
  },
  {
    rank: 6,
    ...
    risk:
      '1）「疲勞」是情緒與利差，不等于 GPU 出貨或台積電營收立刻下滑。2）不同發行人信用品質差很大，不能把單一冷門債券認購不足外推到全體 hyperscaler。3）若輝達財報現金流與預訂量仍強，債市雜音可能一周內被股價忽略。4）路透整理稿可能混用「計畫融資額」與「已發行額」。操作上應等實際新債定價與 8/26 財報，而不是因標題做空整段 AI 鏈。',
    tone: 'bad',
  },
  {
    rank: 7,
    ...
    risk:
      '1）僅「in talks」，交易結構未定，取消機率不低。2）韓國產業政策傾向扶植本土 NPU，外資控股可能觸發審查。3）美國出口管制若把特定加速器視同受控項目，跨境交易文件會拖很久。4）即使成交，對台積電短線營收貢獻難以量化，不宜當 2330 隔日催化。注意韓媒與彭博後續是否改口成「策略合作」而非股權交易。',
    tone: 'good',
  },
  {
    rank: 8,
    ...
    risk:
      '1）「投資」與「獨家洽談」在路透／WSJ／TechCrunch 用詞不完全一致，金額與持股未完整披露。2）電力與土地案高度依賴地方許可，美國中期選舉下資料中心政治阻力上升（州長、居民、電價）。3）專案延遲不會立刻反映在輝達 GPU 出貨，但會讓「保證供電」的敘事打折。4）若結構是擔保或或有負債，會計與信用評等影響可能大於帳上投資。等公司或 Cloverleaf 官方稿再對金額。',
    tone: 'good',
  },
  {
    rank: 9,
    ...
    risk:
      '1）「數百人」相對蘋果總人力很小，媒體容易寫成「放棄 AI」。2）裁的是內容／遊戲／特定 Siri 模組，不代表 A 系列 NPU 或明年 iPhone 功能表被砍。3）若同時向 OpenAI／Google 加深授權，短期是費用下降，長期是差異化風險，這層在頭條裡幾乎沒被量化。4）台鏈個股（光學、連接器）與 Vision Pro 營收占比通常不高，避免把裁員新聞映射成相關持股必跌。以蘋果未發的官方說明為準。',
    tone: 'bad',
  },
  {
    rank: 10,
    ...
    risk:
      '1）「earmark／撥付」不等于已到帳或已轉成設備訂單，實際撥款常分年。2）Rapidus 過往時程多次後移，補貼新聞的歷史預測準確度偏低。3）即使量產，初期產能對全球 AI 晶圓供給是邊際，對 2330 營收模型幾乎動不了小數點。4）匯率、日本政治與美國 CHIPS 配套都會改寫金額。短線若有人用這則做空或做多台積電，都屬過度解讀。',
    tone: 'neutral',
  },
];
